...
function buildImage() {
	echo "Створюю Docker образ springsoapclient ..."
...
function startImage() {
	echo "Запускаю Docker образ springsoapclient ..."
	sudo docker run -it --rm -p 8050:8050 --env-file ./service.env springsoapclient:latest
	
}

function removeImage() {
	echo "Видаляю Docker образ springsoapclient ..."
	sudo docker rmi springsoapclient:latest
}
