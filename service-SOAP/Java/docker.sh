...
function buildImage() {
	echo "Створюю Docker образ springsoapservice ..."
...
function startImage() {
	echo "Запускаю Docker образ springsoapservice ..."
	sudo docker run -it --rm -p 8080:8080 --env-file ./service.env springsoapservice:latest
	
}

function removeImage() {
	echo "Видаляю Docker образ springsoapservice ..."
	sudo docker rmi springsoapservice:latest
}
